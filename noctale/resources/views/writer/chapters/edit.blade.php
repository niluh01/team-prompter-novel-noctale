    ...
    <div class="py-12" x-data="{ showPreview: false, previewContent: '' }">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-2xl sm:rounded-2xl border border-gray-100 p-8">
                <form id="chapterForm" action="{{ route('writer.novels.chapters.update', [$novel, $chapter]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-8 flex flex-col md:flex-row gap-6 items-start">
                        <div class="w-full md:w-1/2">
                            ...
                            @if($chapter->image)
                                <div class="mb-3 relative group w-32 h-20">
                                    <img src="{{ asset('storage/' . $chapter->image) }}" class="w-full h-full object-cover rounded-xl shadow-md border border-gray-200">
                                    <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity rounded-xl flex items-center justify-center">
                                        <span class="text-[10px] text-white font-bold">Ganti Gambar</span>
                                    </div>
                                </div>
                            @endif
                            <div class="relative group">
                                <input type="file" name="image" class="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-bold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 transition-all cursor-pointer border border-dashed border-gray-200 rounded-xl p-2 bg-gray-50/50">
                            </div>
                        </div>
                        <div class="w-full md:w-1/2">
                            <label class="block text-gray-700 font-bold mb-3 text-sm flex items-center">
                                ...
                                Impor Naskah Baru (.txt)
                            </label>
                            <input type="file" id="manuscript_upload" accept=".txt" class="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-bold file:bg-green-50 file:text-green-700 hover:file:bg-green-100 transition-all cursor-pointer border border-dashed border-gray-200 rounded-xl p-2 bg-gray-50/50" onchange="readManuscript(this)">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        <div>
                            ...
                            <input type="number" name="chapter_number" value="{{ old('chapter_number', $chapter->chapter_number) }}" class="w-full border-gray-200 rounded-xl shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 transition-all font-semibold" required>
                        </div>
                        <div class="md:col-span-1">
                            ...
                            <select name="publish_status" id="publish_status" class="w-full border-gray-200 rounded-xl shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 transition-all font-semibold" required onchange="toggleScheduledAt()">
                                <option value="published" {{ old('publish_status', $chapter->publish_status) === 'published' ? 'selected' : '' }}>Publikasikan</option>
                                <option value="draft" {{ old('publish_status', $chapter->publish_status) === 'draft' ? 'selected' : '' }}>Simpan sebagai Draft</option>
                                <option value="scheduled" {{ old('publish_status', $chapter->publish_status) === 'scheduled' ? 'selected' : '' }}>Jadwalkan</option>
                            </select>
                        </div>
                        <div id="scheduled_at_container" class="{{ old('publish_status', $chapter->publish_status) === 'scheduled' ? 'block' : 'hidden' }}">
                            <label class="block text-gray-700 font-bold mb-2 text-sm">Waktu Penayangan</label>
                            <input type="datetime-local" name="scheduled_at" id="scheduled_at" value="{{ old('scheduled_at', $chapter->scheduled_at ? \Carbon\Carbon::parse($chapter->scheduled_at)->format('Y-m-d\TH:i') : '') }}" class="w-full border-gray-200 rounded-xl shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 transition-all font-semibold">
                        </div>
                    </div>

                    <div class="mb-8">
                        <label class="block text-gray-700 font-bold mb-3 text-sm">Judul Bab</label>
                        <input type="text" name="title" value="{{ old('title', $chapter->title) }}" placeholder="Judul bab..." class="w-full border-gray-200 rounded-xl shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 transition-all text-lg font-bold py-3 px-4" required>
                    </div>
                    ...
                    <div class="mb-8">
                        ...
                        <div class="rounded-2xl border border-gray-200 overflow-hidden shadow-inner bg-gray-50 text-gray-800">
                            <!-- Hidden input to store Quill content -->
                            <input type="hidden" name="content" id="content_area" value="{{ old('content', $chapter->content) }}">
                    ...
                    <div class="mt-10 flex flex-col sm:flex-row justify-between items-center gap-4 text-sm border-t border-gray-100 pt-8">
                        ...
                        <button type="submit" class="w-full sm:w-auto px-10 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 shadow-lg shadow-blue-200 transition-all transform hover:-translate-y-0.5 active:translate-y-0">
                            Simpan Perubahan
                        </button>
                    </div>
                </form>
